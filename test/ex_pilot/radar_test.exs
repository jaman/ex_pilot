defmodule ExPilot.RadarTest do
  ...
  test "with playersonradar off only the player's own ship is marked" do
    ...
    view = %{
      ...
      radar: %{players?: false},
      ships: [
        %{id: :a, pos: {2.5, 2.5}, alive?: true, team: nil},
        %{id: :b, pos: {6.5, 6.5}, alive?: true, team: nil}
      ]
    }

    colours =
      Radar.rows(arena, view, {4, 2}) |> List.flatten() |> Enum.map(&elem(&1, 1)) |> Enum.uniq()
  ...
  test "the walls of a big map are scaled once, so a frame's radar costs the marks alone" do
    rows =
      for y <- 0..299,
          do: for(x <- 0..299, do: if(rem(x * y, 7) == 0, do: "x", else: " ")) |> Enum.join()
    ...
    view = %{
      me: %{id: :a, team: nil},
      ships: [%{id: :a, pos: {5.0, 5.0}, alive?: true, team: nil}]
    }
  ...
  test "a ship is a two-by-two mark, a base a single dot, so ships stand out on a big map" do
    ...
    view = %{
      me: %{id: :a, team: nil},
      ships: [%{id: :b, pos: {4.0, 4.0}, alive?: true, team: nil}]
    }

    dots = fn {columns, rows} ->
      for row <- Radar.rows(arena, view, {columns, rows}),
          {text, colour} <- row,
          cell <- String.to_charlist(text),
          cell != 0x2800,
          do: {colour, cell}
    end
    ...
    lit = fn colour ->
      dots.({8, 4})
      |> Enum.filter(&(elem(&1, 0) == colour))
      |> Enum.map(fn {_, cell} -> (cell - 0x2800) |> Integer.digits(2) |> Enum.sum() end)
      |> Enum.sum()
    end
