defmodule ExPilot.RulesTest do
  ...
  defp arena(options) do
    header =
      Enum.map_join(
        [mapwidth: 24, mapheight: 12, edgewrap: "no", gravity: 0, teamplay: "yes"] ++ options,
        "\n",
        fn {k, v} -> "#{k}: #{v}" end
      )
    ...
    %{
      ...
      | ships:
          Map.put(
            ...
            Map.merge(
              Ship.timer(
                %{s | body: body, landed?: Keyword.get(opts, :landed?, false)},
                :immune_until,
                0.0
              ),
  ...
  defp shoot(game, shooter, steps \\ 30),
    do: game |> holding(shooter, [:fire]) |> play(1) |> holding(shooter, []) |> play(steps)
  ...
  describe "fuel" do
    test "a ship with an empty tank, flying or on its base, is lost after ten seconds and loses a life" do
      ...
      assert Enum.any?(Game.view(lost, :b).messages, fn {_, text} ->
               text =~ "a ran out of fuel"
             end)
    ...
    test "when every ship is out of fuel the round ends with nobody standing" do
      ...
      game =
        game
        |> place(:a, {6.0, 8.0}, with: [fuel: 0.0])
        |> place(:b, {18.0, 8.0}, with: [fuel: 0.0])
  ...
  describe "the ball on its string" do
    test "a carried ball trails on a string, swings with its own velocity and is pulled by gravity" do
      ...
      [%{pos: {bx, by}, vel: {vx, _}, string: {sx, _}}] =
        Enum.filter(Game.view(away, :a).balls, &(&1.string != nil))
    ...
    test "a loose ball keeps moving, bounces off walls and comes to rest" do
      ...
      rolling =
        %{game | balls: [%{left | pos: {3.5, 5.5}, vel: {-8.0, 0.0}, carrier: nil} | rest]}
        |> play(20)
    ...
    test "with ballswallbounce off, a ball that hits a wall goes home" do
      ...
      home =
        %{game | balls: [%{left | pos: {2.0, 5.5}, vel: {-8.0, 0.0}, carrier: nil} | rest]}
        |> play(20)
  ...
  describe "watching" do
    test "a player out of lives follows the nearest ship and steps through the others with next_watch" do
      game =
        game(limitedlives: "yes", worldlives: 1)
        |> joined(:a, %{team: 1})
        |> joined(:b, %{team: 2})
        |> joined(:c, %{team: 1})
        |> joined(:d, %{team: 2})
  ...
  describe "map options" do
    test "allowshields off leaves the shield key dead" do
      ...
      refute Enum.any?(
               Game.view(game |> holding(:b, [:shield]) |> play(1), :a).ships,
               &(&1.id == :b and &1.shielding?)
             )
    end
    ...
    test "initialfuel and the initial item counts fill a new ship" do
      ...
      assert ship(Game.step(%{game | ships: Map.put(game.ships, :a, Ship.die(s, 0.0))}, @dt), :a).items.mine ==
               2
    end

    test "shotswallbounce lets shots bounce off walls; shotsgravity off keeps them straight" do
      bouncing =
        game(shotswallbounce: "yes")
        |> joined(:a)
        |> place(:a, {21.0, 6.0}, heading: 0)
        |> holding(:a, [:fire])
        |> play(1)
        |> holding(:a, [])
        |> play(20)
      ...
      straight =
        game(gravity: -5.0, shotsgravity: "no")
        |> joined(:a)
        |> place(:a, {5.0, 6.0}, heading: 0, landed?: true)
        |> holding(:a, [:fire])
        |> play(1)
        |> holding(:a, [])
        |> play(10)
    ...
    test "ships that fly into each other crash, bounce when both are shielded, and pass when crashes are off" do
      meet = fn options, shields ->
        game(options)
        |> joined(:a)
        |> joined(:b, %{team: 2})
        |> place(:a, {10.0, 6.0}, vel: {6.0, 0.0})
        |> place(:b, {11.5, 6.0}, vel: {-6.0, 0.0})
        |> holding(:a, shields)
        |> holding(:b, shields)
        |> play(10)
      end
        ...
        | targets: targets,
          shots: [
            %{
              ...
              owner: :a,
              kind: :shot,
              body: Cauldron2D.Body.new(pos: {21.5, 7.5}, vel: {0.0, 0.0}, radius: 0.1),
              life: 1.0
            }
          ]
      }
    ...
    test "dropitemonkillprob drops a killed ship's items on the floor" do
      game =
        game(dropitemonkillprob: 1.0, itemprobmult: 0)
        |> joined(:a)
        |> joined(:b, %{team: 2})
        |> place(:a, {10.0, 6.0}, heading: 0)
        |> place(:b, {14.0, 6.0}, with: [items: %{mine: 2, laser: 1}])
      ...
      kept =
        game(dropitemonkillprob: 0.0, itemprobmult: 0)
        |> joined(:a)
        |> joined(:b, %{team: 2})
        |> place(:a, {10.0, 6.0}, heading: 0)
        |> place(:b, {14.0, 6.0}, with: [items: %{mine: 2}])
        |> shoot(:a)
    ...
    test "gravityangle turns uniform gravity and gravitypointsource pulls toward gravitypoint" do
      sideways =
        game(gravity: -2.0, gravityangle: 0)
        |> joined(:a)
        |> place(:a, {10.0, 6.0})
        |> play(25)
        |> ship(:a)
      ...
      point =
        game(gravity: -2.0, gravitypoint: "20,6", gravitypointsource: "yes")
        |> joined(:a)
        |> place(:a, {10.0, 6.0})
        |> play(25)
        |> ship(:a)
    ...
    test "a shielded ship picks up no item and drops no mine unless the map allows it" do
      with_item = fn options ->
        %{game(options) | items: [%{pos: {10.5, 6.5}, kind: :laser, ttl: 60.0}]}
        |> joined(:a)
        |> place(:a, {10.4, 6.5})
        |> holding(:a, [:shield])
        |> play(2)
    ...
    test "checkpointradius widens a race checkpoint" do
      s = %{
        Ship.new(:a, "a", {1, 1})
        | body: %{Ship.new(:a, "a", {1, 1}).body | pos: {14.0, 2.5}}
      }

      assert ExPilot.Race.pass(s, [{12, 2}], 2, 3.0).race.laps == 1
      assert ExPilot.Race.pass(s, [{12, 2}], 2).race.laps == 0
    end
