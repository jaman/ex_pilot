defmodule ExPilot.FeaturesTest do
  ...
  defp place(game, id, pos, opts \\ []) do
    ...
    %{game | ships: Map.put(game.ships, id, Map.merge(%{s | body: body, landed?: false, immune_until: 0.0}, Map.new(Keyword.get(opts, :with, []))))}
  end
  ...
  describe "items" do
    test "lie on the map, are picked up by flying over them, and go into the inventory" do
      game = %{game() | items: [%{pos: {8.5, 3.5}, kind: :afterburner, ttl: 60.0}, %{pos: {12.5, 3.5}, kind: :fuel, ttl: 60.0}]} |> joined(:a)
  ...
  describe "using items" do
    test "afterburner thrusts harder, armour absorbs a shot, cloak hides the ship from others" do
      plain = game() |> joined(:a) |> place(:a, {10.0, 6.0}, heading: 0) |> holding(:a, [:thrust]) |> play(25)
      burning = game() |> joined(:a) |> place(:a, {10.0, 6.0}, heading: 0, with: [items: %{afterburner: 1}]) |> holding(:a, [:thrust]) |> play(25)
      assert elem(ship(burning, :a).body.vel, 0) > elem(ship(plain, :a).body.vel, 0)

      game = game() |> joined(:a) |> joined(:b, %{team: 2})
      game = game |> place(:a, {10.0, 6.0}, heading: 0) |> place(:b, {14.0, 6.0}, heading: 32, with: [armour: 1])
      game = game |> holding(:a, [:fire]) |> play(1) |> holding(:a, []) |> play(30)
      assert ship(game, :b).alive?
      assert ship(game, :b).armour == 0

      cloaked = game() |> joined(:a) |> joined(:b, %{team: 2}) |> place(:a, {10.0, 6.0}, with: [items: %{cloak: 1}]) |> holding(:a, [:cloak]) |> play(2)
    ...
    test "a mine waits, then kills the next ship over it but not its owner at once" do
      game = game() |> joined(:a) |> joined(:b, %{team: 2}) |> place(:a, {10.0, 6.0}, with: [items: %{mine: 1}]) |> place(:b, {16.0, 6.0}, heading: 32, vel: {-4.0, 0.0})
    ...
    test "a smart missile turns toward the nearest enemy and a laser hits the first ship in line" do
      game = game() |> joined(:a) |> joined(:b, %{team: 2}) |> place(:a, {10.0, 6.0}, heading: 0, with: [items: %{missile: 2}, missile: :smart]) |> place(:b, {14.0, 3.0})
      ...
      lasered = game() |> joined(:a) |> joined(:b, %{team: 2}) |> place(:a, {10.0, 6.0}, heading: 0, with: [items: %{laser: 1}]) |> place(:b, {15.0, 6.0}) |> holding(:a, [:fire_laser]) |> play(1)
    ...
    test "hyperjump moves the ship, phasing passes walls, emergency shield holds without fuel" do
      jumped = game() |> joined(:a) |> place(:a, {10.0, 6.0}, with: [items: %{hyperjump: 1}]) |> holding(:a, [:hyperjump]) |> play(1)
      assert ship(jumped, :a).body.pos != {10.0, 6.0}

      phased = game() |> joined(:a) |> place(:a, {21.0, 6.0}, heading: 0, vel: {10.0, 0.0}, with: [items: %{phasing: 1}]) |> holding(:a, [:phasing]) |> play(15)
      assert ship(phased, :a).alive?
      assert elem(ship(phased, :a).body.pos, 0) > 23.0

      shielded = game() |> joined(:a) |> joined(:b, %{team: 2}) |> place(:a, {10.0, 6.0}, heading: 0) |> place(:b, {14.0, 6.0}, with: [fuel: 0.0, items: %{emergency_shield: 1}])
      shielded = shielded |> holding(:b, [:emergency_shield]) |> holding(:a, [:fire]) |> play(1) |> holding(:a, []) |> play(30)
  ...
  describe "spawning" do
    test "a ship that has just appeared on its base cannot be hit for three seconds, and shows a shield meanwhile" do
      ...
      assert ship(revived, :b).immune_until > revived.time
    end
  end

  describe "teams" do
    test "teammates' shots pass through each other and team scores add up" do
      game = game() |> joined(:a, %{team: 1}) |> joined(:b, %{team: 1}) |> place(:a, {10.0, 6.0}, heading: 0) |> place(:b, {14.0, 6.0})
      game = game |> holding(:a, [:fire]) |> play(1) |> holding(:a, []) |> play(30)
      assert ship(game, :b).alive?
  ...
  describe "targets, balls and the race" do
    ...
    test "a ship picks up a ball, carries it home and scores for its team" do
      ...
      delivered = game |> place(:a, {21.5, 5.5}, with: [ball: left.id]) |> ball_at(left.id, {20.5, 5.5}) |> play(3)
    ...
    test "a racer passes checkpoints in order and finishes after the laps" do
      ...
      assert passed.laps == 1 and not passed.finished?
  ...
  describe "messages" do
    test "kills and crashes are told in the arena" do
      game = game() |> joined(:a) |> joined(:b, %{team: 2}) |> place(:a, {10.0, 6.0}, heading: 0) |> place(:b, {14.0, 6.0})
      game = game |> holding(:a, [:fire]) |> play(1) |> holding(:a, []) |> play(30)
