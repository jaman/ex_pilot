defmodule ExPilot.Sound do
  ...
  def voice(:missile), do: Voice.new(shape: :saw, freq: 220.0, sweep: 0.5, cutoff: 0.4, envelope: Envelope.hit(0.3), gain: 0.25)
  def voice(:mine), do: Voice.new(shape: :square, freq: 110.0, envelope: Envelope.hit(0.15), gain: 0.2)
  def voice(:laser), do: Voice.new(shape: :saw, freq: 1760.0, sweep: 0.2, envelope: Envelope.hit(0.12), gain: 0.2)
  def voice(:cloak), do: run([880.0, 659.25, 440.0], 0.1, 0.2)
  def voice(:ecm), do: Voice.new(shape: :noise, highpass: 0.3, envelope: Envelope.hit(0.4), gain: 0.2)
  def voice(:transporter), do: run([440.0, 880.0, 1760.0], 0.08, 0.2)
  def voice(:hyperjump), do: Voice.new(shape: :sine, freq: 330.0, sweep: 3.0, envelope: Envelope.hit(0.25), gain: 0.25)
  def voice(:target_hit), do: Voice.new(shape: :square, freq: 330.0, envelope: Envelope.hit(0.06), gain: 0.18)
