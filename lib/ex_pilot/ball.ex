defmodule ExPilot.Ball do
  ...
  defp advance(%{carrier: nil} = ball, ships, dt, env) do
    ...
    taker =
      ...
      |> Map.values()
      |> Enum.find(fn ship ->
        ship.alive? and ship.ball == nil and ball.id not in carrying and
          not (at_home?(ball) and ship.team == ball.team) and
          within?(ship.body.pos, ball.pos, @reach)
      end)
  ...
  defp hang(ball, ship, ships, dt, env) do
    ...
    cond do
      ...
      distance > @string ->
        ...
        pulled = %{
          ball
          | vel: add(ball.vel, {-ux * @spring * stretch * dt, -uy * @spring * stretch * dt})
        }
        ...
        tugged = %{
          ...
          | body: %{
              ship.body
              | vel:
                  {vx + ux * @spring * @ship_pull * stretch * dt,
                   vy + uy * @spring * @ship_pull * stretch * dt}
            }
        }

        {pulled |> accelerate(env.fields, @carried_drag, dt) |> travel(dt, env),
         Map.put(ships, ship.id, tugged)}
  ...
  defp accelerate(ball, fields, drag, dt) do
    body =
      Body.new(pos: ball.pos, vel: ball.vel, radius: @radius)
      |> Body.gravitate(fields, dt)
      |> Body.drag(drag, dt)
  ...
  defp travel(ball, dt, env) do
    ...
    case Collision.sweep(env.grid, ball.pos, to, @radius) do
      {:clear, at} ->
        %{ball | pos: wrapped(at, env)}

      {:blocked, at, normal, _cell} when env.bounce? ->
        %{ball | pos: wrapped(at, env), vel: Collision.bounce(ball.vel, normal, @restitution)}

      {:blocked, _at, _normal, _cell} ->
        home(ball)
    end
  end
  ...
  defp home(%{home: {x, y}} = ball),
    do: %{ball | pos: {x + 0.5, y + 0.5}, vel: {0.0, 0.0}, carrier: nil}
  ...
  """
  @spec deliver([ball()], %{term() => map()}, Arena.t()) ::
          {[ball()], [{term(), integer(), pos_integer(), integer() | nil}]}
  def deliver(balls, ships, arena) do
    treasures = new(arena)

    Enum.map_reduce(balls, [], fn ball, scored ->
      case delivered(ball, ships, treasures) do
        {carrier, team} -> {home(ball), [{carrier, team, @score, ball.team} | scored]}
        nil -> {ball, scored}
      end
    end)
  end

  defp delivered(ball, ships, treasures) do
    with carrier when carrier != nil <- ball.carrier,
         %{team: team, body: %{pos: at}} when team != nil and team != ball.team <-
           Map.get(ships, carrier),
         true <- at_a_treasure?(treasures, team, at) do
      {carrier, team}
    else
      _ -> nil
    end
  ...
  defp at_a_treasure?(treasures, team, at),
    do: Enum.any?(treasures, fn t -> t.team == team and within?(at, t.pos, @reach) end)
