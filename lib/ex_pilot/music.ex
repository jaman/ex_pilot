defmodule ExPilot.Music do
  ...
  def pieces do
    for {name, bpm, sections} <- scores() do
      chains = Map.new(sections, fn {section, {_bars, layers}} -> {section, Map.new(layers, fn {layer, chain} -> {layer, polish(layer, chain)} end)} end)
      {name, %{bpm: bpm, beats_per_bar: 4, layers: @layers, sections: chains}}
    end
  end
  ...
  def polish(layer, chain) do
    chain = if layer == :pad and not String.contains?(chain, ".attack("), do: chain <> @pad_swell, else: chain

    case Map.fetch(@room, layer) do
      {:ok, room} -> if String.contains?(chain, ".room("), do: chain, else: chain <> ".room(#{room})"
      :error -> chain
    end
  end

  @doc "Which piece, section and layer levels a player's view calls for on `screen`."
  @spec cue(map() | nil, atom()) :: Cauldron2D.Audio.Music.cue()
  def cue(_view, :title), do: %{piece: "drift", section: :drift, layers: %{drums: 0.0, bass: 0.8, pad: 1.0, lead: 0.9}}
  def cue(_view, :lobby), do: %{piece: "drift", section: :drift, layers: %{drums: 0.7, bass: 0.9, pad: 1.0, lead: 0.7}}
  def cue(view, :settings), do: cue(view, :lobby)

  def cue(%{me: %{alive?: alive?, lives: lives}}, :summary) when alive? or lives == :unlimited or lives > 0,
    do: %{piece: "laurels", section: :victory, layers: %{drums: 1.0, bass: 1.0, pad: 1.0, lead: 1.0}}

  def cue(_view, :summary), do: %{piece: "ashes", section: :defeat, layers: %{drums: 0.8, bass: 1.0, pad: 1.0, lead: 0.9}}

  def cue(view, :arena) do
    case view do
      %{me: %{alive?: false, lives: 0}} -> %{piece: "adrift", section: :out, layers: %{drums: 0.0, bass: 0.9, pad: 1.0, lead: 0.8}}
      %{me: %{fuel: fuel}} when fuel < 150 -> %{piece: "ember", section: :low_fuel, layers: %{drums: 1.0, bass: 1.0, pad: 0.9, lead: 1.0}}
      %{me: %{id: _} = me, ships: ships} -> arena_cue(me, ships, mode_of(view))
      _watching -> cruise(mode_of(view), 0.6)
    end
  end

  @arena_pieces %{dogfight: {"orbit", "burn"}, ctf: {"banner", "raid"}, team: {"phalanx", "siege"}, race: {"circuit", "overtake"}}

  defp mode_of(%{mode: mode}) when is_atom(mode) and mode != nil, do: mode
  defp mode_of(_view), do: :dogfight

  defp arena_cue(me, ships, mode) do
    nearby = Enum.count(ships, fn ship -> ship.id != me.id and within?(me.pos, ship.pos, 14.0) end)
    {_cruise, combat} = Map.fetch!(@arena_pieces, mode)

    cond do
      nearby >= 2 -> %{piece: combat, section: :combat, layers: %{drums: 1.0, bass: 1.0, pad: 0.6, lead: 1.0}}
      nearby == 1 -> %{piece: combat, section: :combat, layers: %{drums: 0.9, bass: 1.0, pad: 0.8, lead: 0.7}}
      true -> cruise(mode, 0.7)
    end
  end
  ...
  defp scores do
    own() ++ ExPilot.Music.Ctf.scores() ++ ExPilot.Music.Team.scores() ++ ExPilot.Music.Race.scores()
  end
  ...
  defp drift do
    ...
    {48,
     %{
       drums: ~s|s("~ hh ~ hh").bank("RolandTR808").gain("<.25 .25 .35 .25>").mask("<1!48>").late(.02)|,
       bass: ~s|note("#{roots}").s("gm_acoustic_bass").gain(.5).clip(.5).release(.3)|,
       pad: ~s|chord("#{chords}").voicing().s("gm_pad_warm").attack(1.2).release(2).clip(.75).lpf(900).gain(.12)|,
       lead: ~s|note("#{cycles([motif_a, motif_b, motif_a, motif_b, motif_c, motif_c])}").s("gm_vibraphone").release(.6).gain("<.4!32 .45!16>").delay(.3).delaytime(.375).delayfeedback(.35).room(.45)|
     }}
  end
  ...
  defp orbit do
    ...
    motif = "[b4 e5 g5@2] [fs5@2 e5 b4] [d5@2 e5@2] [~@2 g5 a5] [b5@2 a5 g5] [fs5@3 ~] [e5@2 g5@2] [b4@3 ~]"
    answer = "[c5 e5 a5@2] [g5@2 e5@2] [~@2 f5 a5] [c6@2 b5@2] [g5@3 ~] [e5 g5 b5@2] [a5@2 g5@2] [e5@3 ~]"
    high = "[b5 e6 g6@2] [fs6@2 e6 b5] [d6@2 e6@2] [~@2 g6 a6] [b6@2 a6 g6] [fs6@3 ~] [e6@2 g6@2] [b5@3 ~]"

    {64,
     %{
       drums:
         ~s|s("<[~ hh]*4!8 [bd hh sd [hh bd]]!16 [bd hh sd [hh bd]]!16 [bd ~ hh ~]!8 [bd hh sd [hh bd]]!16>").bank("RolandTR909").gain("<.35!8 .6!16 .65!16 .35!8 .7!16>")|,
       bass: ~s|note("#{roots}").s("gm_electric_bass_finger").struct("<[x ~ x ~]!8 [x ~ x x]!48 [x ~ x ~]!8>").gain(.45).clip(.5).release(.15)|,
       pad: ~s|chord("#{chords}").voicing().s("gm_epiano2").attack(.05).release(1.5).clip(.9).lpf(2400).gain(.14)|,
       lead: ~s|note("#{cycles(["~!8", motif, motif, answer, answer, "~!8", high, high])}").s("gm_epiano1").gain(.28).release(.4).pan("<.3 -.3>").room(.35)|
     }}
  end
  ...
  defp burn do
    ...
    {48,
     %{
       ...
       bass: ~s|note("#{roots}").s("gm_electric_bass_pick").struct("x*8").gain(.55).clip(.5).lpf(900)|,
       pad: ~s|chord("#{chords}").voicing().s("gm_pad_poly").attack(.3).release(.8).clip(.8).lpf(1600).gain(.1)|,
       lead:
         ~s|n("<[0 4 7 4 0 4 12 4]!16 [7@2 4 0@3 7 4]!8 [12 4 0 4 14 4 0 12]!16 [0 ~@7]!4 [0 1 0 -2 0 1 3 7]!4>").scale("E4:minor").s("gm_lead_2_sawtooth").gain(.8).clip(.6)|
     }}
  end
  ...
  defp ember do
    ...
    {32,
     %{
       drums: ~s|s("[bd hh ~ hh ~ hh rim hh]").bank("RolandTR707").gain(.5).sometimesBy(.2, x => x.speed(1.5))|,
       bass: ~s|note("#{roots}").s("gm_electric_bass_pick").struct("<[x x x x]!3 [x x x [x f2]]>").gain(.55).lpf(600)|,
       pad: ~s|chord("#{chords}").voicing().s("gm_pad_bowed").attack(.8).release(1.2).clip(.8).lpf(1200).gain(.11)|,
       lead: ~s|note("<[e5@2 d5@2] [c5@2 b4@2] ~ [f5@2 e5@2] ~ ~ ~ ~>").s("gm_lead_8_bass_lead").gain(.35).lpf(1200)|
     }}
  end
  ...
  defp adrift do
    ...
    {32,
     %{
       drums: ~s|s("~").gain(0)|,
       bass: ~s|note("<a1!4 f1!4 c2!4 g1!4 a1!4 f1!4 c2!4 g1!4>").s("gm_contrabass").gain(.25).clip(4)|,
       pad: ~s|chord("#{chords}").voicing().s("gm_pad_choir").attack(1.5).release(2).clip(.85).lpf(1400).gain(.12)|,
       lead: ~s|note("<e5 ~ c5 ~ a4 ~ b4 ~ e5 ~ g5 ~ e5 ~ d5 ~>").s("gm_music_box").gain(.4).delay(.4).delaytime(.5).delayfeedback(.45).room(.7)|
     }}
  end
  ...
  defp laurels do
    ...
    fanfare = "[e5 gs5 b5@2 e6@4] [ds6@2 b5@2 cs6@4] [~ a5 b5 cs6@2 b5@2 gs5@2] [e5@4 ~@4] [e5 fs5 gs5@2 b5@4] [e6@8] [~@6 b5@2] [e6@8]"

    {24,
     %{
       drums: ~s|s("[bd hh sd [hh bd]]").bank("RolandTR909").gain(.65).superimpose(x => x.s("cr").gain(.4).mask("<1 0!7>"))|,
       bass: ~s|note("#{roots}").s("gm_electric_bass_finger").struct("[x ~ x x] [x ~ x ~]").gain(.5)|,
       pad: ~s|chord("#{chords}").voicing().s("gm_string_ensemble_1").attack(.5).release(1.0).clip(.8).lpf(1800).gain(.13)|,
       lead: ~s|note("#{cycles([fanfare, fanfare, fanfare])}").s("gm_brass_section").gain(.45).room(.4)|
     }}
  end
  ...
  defp ashes do
    ...
    {24,
     %{
       ...
       pad: ~s|chord("#{chords}").voicing().s("gm_string_ensemble_1").attack(.5).release(1.0).clip(.8).lpf(1800).gain(.13)|,
       lead: ~s|note("#{cycles([lament, lament, lament])}").s("gm_oboe").gain(.4).delay(.25).delaytime(.75).delayfeedback(.3).room(.6)|
     }}
  end
